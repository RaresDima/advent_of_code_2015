...
class Instruction:
    ...
    def apply_to_grid(self, grid: np.ndarray):
        if self.type == Instruction.Type.TURN_ON:
            grid[self.start_y:self.end_y+1, self.start_x: self.end_x+1] = True
        elif self.type == Instruction.Type.TURN_OFF:
            grid[self.start_y:self.end_y+1, self.start_x: self.end_x+1] = False
        else:
            grid[self.start_y:self.end_y+1, self.start_x: self.end_x+1] = \
                 np.logical_not(grid[self.start_y:self.end_y+1, self.start_x: self.end_x+1])
